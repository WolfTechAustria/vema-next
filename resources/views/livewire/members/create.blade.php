<div class="max-w-5xl">
    ...
    <form wire:submit="save" class="space-y-6">
        ...
        @if(!empty($possibleDuplicates) && !$duplicateWarningDismissed)

            <div class="rounded-xl border border-amber-200 bg-amber-50 p-4">

                <div class="text-sm font-semibold text-amber-800">
                    Möglicherweise bereits vorhandenes, ausgetretenes Mitglied gefunden
                </div>

                <p class="mt-1 text-sm text-amber-700">
                    Falls es sich um dieselbe Person handelt, kann statt einer Neuanlage ein Wiedereintritt erfasst werden.
                </p>
                        ...
                        <div class="flex items-center justify-between rounded-lg bg-white px-4 py-2 text-sm">
                            ...
                            <div>
                                ...
                                <span class="text-slate-500">
                                    · geb. {{ $duplicate->dateOfBirth?->format('d.m.Y') ?? 'unbekannt' }}
                                    · ausgetreten am {{ $duplicate->membershipPeriods->first()?->date_to?->format('d.m.Y') ?? 'unbekannt' }}
                                </span>
                            </div>

                            <a
                                href="{{ route('members.show', $duplicate->memberID) }}"
                                target="_blank"
                                class="whitespace-nowrap rounded-lg border border-amber-300 bg-white px-3 py-1.5 text-xs font-semibold text-amber-800 hover:bg-amber-100"
                            >
                                Zum Mitglied (Wiedereintritt dort erfassen)
                            </a>
                ...
                <button
                    type="button"
                    wire:click="dismissDuplicateWarning"
                    class="mt-3 text-xs font-medium text-amber-700 underline hover:text-amber-900"
                >
                    Hinweis ausblenden, trotzdem neu anlegen
                </button>
